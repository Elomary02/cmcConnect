...
class GroupesRvAdapter(private val items: List<Item>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType){
            GroupsFragment.ItemType.HEADER->{
                val view = LayoutInflater.from(parent.context).inflate(R.layout.rv_item_header,parent,false)
                HeaderViewHolder(view)
            }
            GroupsFragment.ItemType.GROUP->{
                val view = LayoutInflater.from(parent.context).inflate(R.layout.rv_formateur_groups_item,parent,false)
